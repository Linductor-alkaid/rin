...
### 2026-09-23：控制行遮挡修复 + 大窗口响应式确认

- 用户反馈：Device 选择器（宽 240，x 106..346）遮挡了 x≈322 的 "Resolution" 标签。
- 修复：Device 选择器收窄左移（x 76，宽 210），Resolution 标签 + 选择器右对齐
  （标签 x = width-278，选择器 x = width-180），两组控件间距 ≥44 逻辑像素。
- 复验：窗口放大至 ~2000px 宽时响应式布局正确（右对齐Resolution 不溢出），
  UI 切换 1280x720 生效（内参 fx 904.9/640.0 同步，截图
  `screenshots/viewer_custom_select_streaming.png` 系列与本次会话留档）。
