// 状态机全矩阵测试（验证代理）：
// 1) isAllowed 全 6x6 矩阵对照独立期望表；
// 2) transitionTo 与 isAllowed 一致性：接受时状态迁移/同状态幂等不变，
//    拒绝时状态不变且 error 文本含 from/to 状态名；
// 3) Failed -> Stopping -> Idle 收敛路径与恢复；Opening 中途 Stopping。
...
constexpr int kStateCount = 6;

// 行 = from，列 = to；行列顺序与 kStates 一致。
// 期望表独立于实现推导：同状态幂等 + 显式转换边
//（Idle->Opening；Opening->{Streaming,Failed,Stopping}；
//  Streaming->{Restreaming,Stopping,Failed}；Restreaming->{Streaming,Failed,Stopping}；
//  Stopping->Idle；Failed->Stopping）。
constexpr bool kExpectedAllowed[kStateCount][kStateCount] = {
    // to:                                Idle Opening Stream Restream Stop Failed
    /* Idle       */ {true, true, false, false, false, false},
    /* Opening    */ {false, true, true, false, true, true},
    /* Streaming  */ {false, false, true, true, true, true},
    /* Restreaming*/ {false, false, true, true, true, true},
    /* Stopping   */ {true, false, false, false, true, false},
    /* Failed     */ {false, false, false, false, true, true},
};
...
constexpr CameraServiceState kStates[kStateCount] = {
    ...
    CameraServiceState::Stopping,   CameraServiceState::Failed};
...
int main() {
    ...
    // --- 4) Failed -> Stopping -> Idle 收敛，收敛后可重新进入 Opening（恢复）---
    {
        CameraStateMachine machine;
    ...
    // --- 5) Opening 中途 Stopping -> Idle（start 阶段取消路径）---
    {
        CameraStateMachine machine;
        RSV_CHECK(machine.transitionTo(CameraServiceState::Opening));
        RSV_CHECK(machine.transitionTo(CameraServiceState::Stopping));
